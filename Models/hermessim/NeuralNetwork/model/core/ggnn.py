...
class EmbeddingEncoder(torch.nn.Module):
    ...
    def forward(self, x, e):
        current_max_x = x.max().item()
        current_max_e = e.max().item()

        self.max_x = max(self.max_x, current_max_x)
        self.max_e = max(self.max_e, current_max_e)

        
        # print(f"Current max X: {current_max_x}, Historical max X: {self.max_x}")
        # print(f"Current max E: {current_max_e}, Historical max E: {self.max_e}")

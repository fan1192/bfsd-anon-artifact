...
def token_mapping(input_folder, output_dir, freq_mode=True):
    ...
    if len(not_cached_graph_types) != 0:
        ...
        for f_json in tqdm(os.listdir(input_folder)):
            ...
            if arch == 'a':
                ...
                arch = 'x'
            # arch_part = f_json.split('-', 1)[0]  # Split on the first hyphen
            # if arch_part.startswith('mips'):
            #     arch = 'mips'
            # elif arch_part.startswith('arm'):
            #     arch = 'arm'
            # elif arch_part.startswith(('x86', 'x64', 'x')):
            #     arch = 'x'
            # else:
            #     # Handle unexpected architectures, perhaps raise an error or log a warning
            #     print("arch_part")
            #     print(arch_part)
            #     raise ValueError(f"Unsupported architecture in filename: {f_json}")
                ...
                for gtype in not_cached_graph_types:
                    ...
                    for bb in fva_data['nverbs']:
                        ...
                        for ty, opc in process_nverb(gtype, nverb, arch):
                            if gtype not in opc_counters:
                                opc_counters[gtype] = {}  # Initialize the outer dictionary for gtype

                            if ty not in opc_counters[gtype]:
                                opc_counters[gtype][ty] = Counter() # Initialize the inner dictionary for ty
                    ...
                    for ty, opc_set in opc_sets.items():
                        ...
                        print("[D] gtype: ", gtype)
                        if gtype not in opc_counters:
                            opc_occurs[gtype] = {}  # Initialize the outer dictionary for gtype
                            

                        if ty not in opc_counters[gtype]:
                            opc_occurs[gtype][ty] = defaultdict(set)
                                            
                        # if ty not in opc_occurs[gtype]:
                        #     opc_occurs[gtype][ty] = set()  
        ...
        # def materialize_sets(obj):
        #     if isinstance(obj, dict):
        #         return {k: materialize_sets(v) for k, v in obj.items()}
        #     if isinstance(obj, set):
        #         return sorted(obj)  # or list(obj) if you don’t care about order
        #     return obj
        ...
        #     # Dump counters (already JSON-safe)
        #     output_path = os.path.join(sub_dir, "opc_counter.json")
        #     with open(output_path, "w") as f:
        #         json.dump(opc_counters[gtype], f, indent=2, ensure_ascii=False)

        #     # Dump occurs (convert sets -> lists first)
        #     output_path = os.path.join(sub_dir, "opc_occurs.json")
        #     opc_occurs[gtype]["num_funcs"] = num_func
        #     with open(output_path, "w") as f:
        #         json.dump(materialize_sets(opc_occurs[gtype]), f, indent=2, ensure_ascii=False)
    ...
    #     DEFAULT_FLOAT_TH = 0.01
    #     OPC_CAP = 50 if gtype == 'ACFG' else 55

    #     # Base thresholds
    #     ths = {
    #         'opc': OPC_CAP,
    #         'val': DEFAULT_FLOAT_TH,
    #         **{f'{arch}_reg': DEFAULT_FLOAT_TH for arch in ['mips', 'arm', 'x']},
    #     }

    #     # Auto-add any reg-like or unknown types present in opc_cnts
    #     for ty_key in opc_cnts.keys():
    #         if ty_key == 'opc':
    #             continue
    #         if ty_key not in ths and (
    #             ty_key.endswith('_reg')
    #             or 'reg' in ty_key
    #             or ty_key in {'consum_reg', 'prod_reg', 'def_reg', 'use_reg'}
    #         ):
    #             ths[ty_key] = DEFAULT_FLOAT_TH

    #     for ty, opc_cnt in opc_cnts.items():
    #         if ty.endswith("_occur"):
    #             continue

    #         # normalize opc_cnt to a sorted list of (key, count)
    #         if not isinstance(opc_cnt, dict):
    #             opc_cnt = [(k, v) for k, v in opc_cnt.most_common()]
    #         else:
    #             opc_cnt = sorted(opc_cnt.items(), key=lambda kv: kv[1], reverse=True)
    ...
    #         # fetch threshold safely with default
    #         th = ths.get(ty, DEFAULT_FLOAT_TH)
    ...
    #                 if (not freq_mode and tot_cnt > 0 and mapped_cnt / tot_cnt > th) \
    #                 or (freq_mode and num_func > 0 and v / num_func < th):
    #                     break
    #             elif isinstance(th, int) and i + 1 >= th:
    #                 break
...
def main(input_dir, training, freq_mode, opcodes_json, output_dir, dataset, out_format):
    ...
    for gtype, g_str_dict in str_dict.items():
        ...
        with open(output_path, 'w') as f_out:
            json.dump(convert_ndarray_to_list(g_str_dict), f_out)

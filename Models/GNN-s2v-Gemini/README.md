...
```bash
# For the ML model in "Neural network-based graph embedding for cross-platform binary code similarity detection." (Gemini) 
--network_type annotations      --features_type none
--network_type annotations      --features_type numerical 

# For the ML models in "Investigating Graph Embedding Neural Networks with Unsupervised Features Extraction for Binary Analysis." (Massarelli et al.)

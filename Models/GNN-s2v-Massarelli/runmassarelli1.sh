...
INSTR_EMB_DIR="$PRETRAIN_DIR/${DATASET_NAME}_training"
BASE="$PROJECT_ROOT/DBs/${DATASET_NAME}/features"

########################################
# Clean old preprocessing outputs (no sudo)
########################################

find "$PREPROCESS_DIR" -type d -name "*${DATASET_NAME}*" -exec rm -rf {} +

########################################
# TRAINING (SAFE pretraining -> instruction embeddings -> preprocessing)
########################################

start=$(date +%s)

# 1) SAFE pretraining (generates ins2id.json etc.)
docker run --rm \
  -v "$BASE/training/acfg_disasm_${DATASET_NAME}_training:/input" \
...
# 2) Convert instructions to embeddings / graph preprocessing
docker run --rm \
  -v "$BASE/training/acfg_disasm_${DATASET_NAME}_training:/input" \
  -v "$INSTR_EMB_DIR:/instruction_embeddings" \
  -v "$PREPROCESS_DIR:/output" \
  gnn-s2v-preprocessing \
  /output/digraph_instructions_embeddings.py \
    -i /input \
...
docker run --rm \
  -v "$BASE/validation/acfg_disasm_${DATASET_NAME}_validation:/input" \
  -v "$INSTR_EMB_DIR:/instruction_embeddings" \
  -v "$PREPROCESS_DIR:/output" \
  gnn-s2v-preprocessing \
  /output/digraph_instructions_embeddings.py \
...
docker run --rm \
  -v "$BASE/testing/acfg_disasm_${DATASET_NAME}_testing:/input" \
  -v "$INSTR_EMB_DIR:/instruction_embeddings" \
  -v "$PREPROCESS_DIR:/output" \
  gnn-s2v-preprocessing \
  /output/digraph_instructions_embeddings.py \

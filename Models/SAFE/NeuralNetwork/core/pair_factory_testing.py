...
class PairFactoryTesting(PairFactoryBase):
    ...
    def __init__(self, pos_path, neg_path, feat_path, batch_size, max_ins):
        ...
        log.info("testing factory pos_path Reading {}".format(pos_path))
        self._func_pos = pd.read_csv(pos_path)
        log.info("testing factory neg_path Reading {}".format(neg_path))
        self._func_neg = pd.read_csv(neg_path)
        
        # Load function features
        log.info("testing factory feat_path Loading {}".format(feat_path))
        
        print(format(feat_path))
        ...
        while True:
            ...
            for _ in range(self._num_func_pairs):
                ...
                for pair in [pos, neg]:
                    ...
                    # ###############DEBUG################
                    # if pair['idb_path_1'] not in self._fdict:
                    #     print("!!!!!testing!!!!!"+pair['idb_path_1'])
                    # if pair['idb_path_2'] not in self._fdict:
                    #     print("!!!!!testing!!!!!"+pair['idb_path_2'])
                    # ll.append(tuple(("111", "1111")))
                    # ###############DEBUG################

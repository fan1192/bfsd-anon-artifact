...
class PairFactoryInference(PairFactoryBase):
    ...
    def __init__(self, func_path, feat_path, batch_size, max_ins):
        ...
        # Load function pairs
        log.info("inference func_path Reading {}".format(func_path))
        self._func = pd.read_csv(func_path)

        # Load function features
        log.info("inference feat_path Loading {}".format(feat_path))
    ...
    def _get_next_pair(self):
        ...
        while True:
            ...
            for _ in range(self._num_func_pairs):
                ...
                # ################DEBUG################
                # if r['idb_path_1'] not in self._fdict:
                #     print("!!!!!inference!!!!!"+r['idb_path_1'])
                # if r['idb_path_2'] not in self._fdict:
                #     print("!!!!!inference!!!!!"+r['idb_path_2'])
                # yield (("111", "1111"))  
                # ################DEBUG################ 
